...
|stats avg(temperature_2m) AS temp by city _time
|where city=="Bonn" OR city=="London" OR city=="Stockholm"
|xyseries _time, city, temp`}>
</SplunkSearch>
...
<SplunkSearch earliest="-1y" latest="now"  spl={`index="s4c_meteo" Bonn OR London OR Stockholm 
|timechart  span=4h  avg(temperature_2m) by city`}>
</SplunkSearch>
...
* Would it be better to add "|fields city temperature_2m _time"
after the first line? Check it out:


<SplunkSearch earliest="-1y" latest="now"  spl={`index="s4c_meteo" Bonn OR London OR Stockholm 
| fields city temperature_2m _time 
| timechart  span=4h  avg(temperature_2m) by city`}>
</SplunkSearch>
