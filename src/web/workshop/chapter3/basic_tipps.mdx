...
> **Excursion — which indexes can you actually search?**
>
> Use `tstats` to see every index your role has access to, along with event counts and date ranges:
...
> Now try searching **without** specifying `index=` at all — this shows only your **default search scope**:

<SplunkSearch earliest="-30d" latest="now" spl={`* | stats count by index | sort -count`}></SplunkSearch>

> Notice the difference? Without `index=` you only search the default scope (usually just `main`). Always be explicit!
