...
<LabBlock title="Compare searches">

## Lets look at this "traditional" search in raw weather data: 
<SplunkSearch earliest="-2y" latest="now" spl={`index=s4c_weather | timechart span=1h avg(main.temp) avg(wind.speed) avg(main.humidity) avg(main.pressure) by name limit=0`}></SplunkSearch>

use job inspector and compare it to this "tstats" search: 
<SplunkSearch earliest="-2y" latest="now" spl={`| tstats avg(main.temp) avg(wind.speed) avg(main.humidity) avg(main.pressure) WHERE index=s4c_weather earliest=-2y latest=now() by name _time span=1h | xyseries _time,name,avg*`}></SplunkSearch>

</LabBlock>


## What indexes can I search?

<LabBlock title="Try it out">

All indexes your role can access — sorted by event count:

<SplunkSearch earliest="-10y" latest="now" spl={`| tstats count WHERE index=* OR index=_* BY index`}></SplunkSearch>

What do you get when you search **without** specifying an index? (default search scope only):

<SplunkSearch earliest="-30d" latest="now" spl={`* | stats count by index | sort -count`}></SplunkSearch>
...
<LabBlock title="Filter examples">
