...
<SplunkSearch earliest="-10y" latest="now" spl={`| tstats count earliest(_time) as oldest latest(_time) as newest
    WHERE index=* OR index=_*
    BY index
| eval oldest=strftime(oldest,"%Y-%m-%d"), newest=strftime(newest,"%Y-%m-%d")
| eval type=if(match(index,"^_"),"internal","data")
| sort type -count`}></SplunkSearch>
