...
<SplunkSearch earliest="-1y" latest="now" spl={`index="s4c_meteo" |timechart span=1h avg(temperature_2m) avg(wind_speed_10m) avg(relative_humidity_2m) avg(pressure_msl) by city limit=0`}></SplunkSearch>

use job inspector and compare it to this "tstats" search: 
<SplunkSearch earliest="-1y" latest="now" spl={`|tstats avg(temperature_2m) avg(wind_speed_10m) avg(relative_humidity_2m) avg(pressure_msl) WHERE index=s4c_meteo earliest=-1y latest=now()  by city _time span=1h| xyseries _time,city,avg*`}></SplunkSearch>
