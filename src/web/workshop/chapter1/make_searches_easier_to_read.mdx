...
*On Linux or Windows you could also use `Ctrl + \ .` - but this shortcut only works with US / EN keyboards.  Better use ** `Ctrl + Shift + F` ** which works better with many non-English keyboards
...
Run this search and use the formatting macro: (the search may not produce results in the test environment, but is quite useful to run on license server)
...
<div className="s4c-kbd-callout">
 ...
 <Link to="https://docs.splunk.com/Documentation/Splunk/latest/Search/Parsingsearches#Search_bar_shortcuts" openInNewContext> More Searchbar Shortcuts</Link>
