...
<SplunkSearch earliest="-30d" latest="now" spl={`| mcatalog values(metric_name) WHERE \`s4c_index_internal_metrics\``}></SplunkSearch>

## Using mstats command
<SplunkSearch earliest="-30d" latest="now" spl={`| mstats avg("spl.mlog.thruput.thruput.average_kbps") prestats=true WHERE \`s4c_index_internal_metrics\` span=15m
...
<SplunkSearch earliest="-30d" latest="now" spl={` | mpreview \`s4c_index_internal_metrics\` filter="group=dutycycle name=ingest thread=udp"`}> </SplunkSearch>
